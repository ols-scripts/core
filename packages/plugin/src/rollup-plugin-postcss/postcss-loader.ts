...
function isModuleFile(file, cacheCssModules) {
  ...
  return cacheCssModules?.includes(file)
}
...
export default {
  ...
  async process({ code, map }) {
    const config: any = this.options.config ? await loadConfig(this.id, this.options.config) : {}

    const { options } = this
    const cacheCssModules = options.ctx.cssModules
    ...
    const isAutoModule = autoModules && isModuleFile(this.id, cacheCssModules)
